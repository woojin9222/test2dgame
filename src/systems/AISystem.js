...
import { Position, ColonistState, JobWorker, IdleTimer, IsColonist, Hunger, Energy, } from '../ecs/components';
...
let _pollTimer = 0;
export function aiSystem(ecsWorld, time, delta, jobQueue, worldMap) {
    ...
    for (const eid of eids) {
        ...
        if (job) {
            ...
            if (path.length === 0) {
                ...
                pathIdx.set(eid, 0);
                ColonistState.state[eid] = 1 /* ColonistStateId.Moving */;
...
function _startWander(eid, worldMap) {
    const wx = Position.x[eid];
    const wy = Position.y[eid];
    ...
    for (let attempt = 0; attempt < 12; attempt++) {
        ...
        if (!worldMap.isWalkable(ntx, nty))
            continue;
        ...
        if (path.length > 0) {
            pathMap.set(eid, path);
            pathIdx.set(eid, 0);
            ColonistState.state[eid] = 5 /* ColonistStateId.Wandering */;
            IdleTimer.elapsed[eid] = 0;
            return;
        }
    }

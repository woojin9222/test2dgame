...
export function movementSystem(
  ...
  for (const eid of eids) {
    ...
    const isMoving   = state === ColonistStateId.Moving
    const isWandering = state === ColonistStateId.Wandering

    if (!isMoving && !isWandering) continue
...
function _arrive(eid: number, wasMovingToJob: boolean, jobQueue: JobQueue): void {
  pathMap.delete(eid)
  pathIdx.delete(eid)
  ...
  if (wasMovingToJob) {
    ...
    if (jobId !== -1) {
      ColonistState.state[eid] = ColonistStateId.Working
    } else {

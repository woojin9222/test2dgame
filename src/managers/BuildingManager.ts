...
export class BuildingManager {
  ...
  private _floorTiles: Int32Array = new Int32Array(MAP_W * MAP_H)
  ...
  finishBuilding(eid: number): boolean {
    const kind = Building.kind[eid] as BuildingKind
    const def  = BUILDING_DEFS[kind]

    // Deduct materials NOW (at construction completion, not blueprint placement)
    if (!def.instantBuild) {
      if (this._gameTime.wood  < def.costWood)  return false
      if (this._gameTime.stone < def.costStone) return false
